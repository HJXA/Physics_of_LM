...
"""
iGSM 通用题目生成原型（IdGen_PT）。

主要职责：
- 采样图结构超参数（层深、宽度、边密度、操作数）；
- 构造 Problem 并控制 permutation/detail 风格；
- 将 problem / question / solution / answer 编码为 GPT2 token 序列。
"""
...
class IdGen_PT(object):
    ...
    - `prob_token`: 题目 token；
    - `sol_token`: 解答 token；
    - `ans_token`: 最终答案 token；
    - `token_id`: 完整训练样本 token 序列。
    """
        if not problem:
            while True:
                self.gen_param()

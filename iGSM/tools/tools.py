...
- 随机种子与数值工具；
- 文本骨架化与哈希分桶；
- 图算法辅助（随机拓扑序、子图提取、最短路长度）；
- token 序列展示、标签掩码、表格展示等调试工具。
"""

tokenizer = GPT2Tokenizer.from_pretrained('gpt2')

def fix_seed(seed: int):
    """固定 Python 与 NumPy 随机种子。"""
    random.seed(seed)
    np.random.seed(seed)

def to_sketch(problem, prob: str=None, sol: str=None):
    """将题面/解答归一化为结构骨架文本，用于去表面差异比较。"""
    sketch = {}
...
def show_info(output: List[int], problem, req_return=False):
    """按 iGSM 特殊分隔符解码并打印 problem/solution/answer。"""
    print()
    problem.display()
    pre = 0
    for i in output:
...
def decode_detail_level(detail_level):
    """将 detail_level 整数编码还原为三位规则码字符串。"""

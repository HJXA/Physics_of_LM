...
# - all the changes from ../main/generator.py (but none of them were used in this gla/eval.py procedure)
# - using huggingface tokenizer instead of the lingua tokenizer
# (Note that most of this code is not used in ../main/generator.py, but kept here for consistency)
#
...
def load_consolidated_model_and_tokenizer(
    ...
    if config.data.tokenizer.name=='sp':
        tokenizer = AutoTokenizer.from_pretrained("NousResearch/Llama-2-7b-hf")
    elif config.data.tokenizer.name=='tiktoken':
        tokenizer = AutoTokenizer.from_pretrained("nvidia/Llama-3.1-Nemotron-70B-Instruct-HF")
    else:
        assert False, f"Unsupported tokenizer name: {config.data.tokenizer.name}"

...
# - added support of eval_account/eval_qos/exclude for slurm options
# - added world_size for logging purpose
# - added slurm support for <8 gpu eval job launching
#
...
from apps.gla.transformer import (
    LMTransformerArgs,
    LMTransformer,
...
class TrainArgs:
    ...
    eval_account: str = "physics_of_lm"
    eval_qos: str = "lowest"
    exclude: str = ""
    world_size: int = -1  # for logging purposes, not used in training
...
def validate_train_args(args: TrainArgs, output_size: int):
    ...
    if args.logging.wandb is not None:
        if args.logging.wandb.name is None:
...
def train(args: TrainArgs):
    with ExitStack() as context_stack:
        ...
        if args.checkpoint.init_ckpt_path:
            ...
            if hasattr(model, 'rope_embeddings'):  # Zeyuan's edit note: linear models don't have this
        ...
        while train_state.step < args.steps:
            ...
            if every_n_steps(
                ...
                logger.info(
                    ...
                    f"  percent: {round(train_state.step / args.steps * 100, 2):>5}%"
            ...
            if args.eval is not None and every_n_steps(
                ...
                eval_args.name = f"EVL_{train_state.step}_" + args.name
                ...
                if args.async_eval_gpus is None:
                    ...
                    with clean_env():
                        launch_job(
                            StoolArgs(
                                asdict(eval_args),
                                script="apps.gla.eval",
                                copy_code=False,
                                nodes=nodes,
                                ngpu=ngpu,
                                account=args.eval_account,
                                qos=args.eval_qos,
                                time=23*60,
                                ncpu=2,
            ...
            if preemption_flag["flag"]:
                if not saved: 
                    logger.warning("Begin checkpointing before preemption")
                    checkpoint.save(
                        model,
                        optimizer,
                        train_state,
                    ...
                    logger.warning("Checkpointing done")
                requeue_slurm_job()
                sys.exit(0)

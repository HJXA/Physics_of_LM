...
def main():
    ...
    print(f"开始启动对 {args.num_eval_samples} 个测试样本的准确率评估...")
    ...
    for _ in tqdm(range(args.num_eval_samples)):
        ...
        # ========================================================
        gt_seq = config.generate_onedata_pure(rng)
        # 截取前 args.prefix_len
    ...
    acc_uncond = correct_unconditional / args.num_eval_samples
    acc_cond = correct_conditional / args.num_eval_samples
    
    print("-" * 50)
    print("【评测最终报告】")
    print(f"场景一 (无条件, 即生成完整句子) Accuracy: {acc_uncond * 100:.2f}%")
    print(f"场景二 (有条件, 馈送前 {args.prefix_len} 个 Tokens 且符合语境) Accuracy: {acc_cond * 100:.2f}%")
    print("-" * 50)
